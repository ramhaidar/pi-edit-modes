...
async function fixture(run: (cwd: string, fs: DeepSeekFsParity) => Promise<void>) {
  ...
  try {
    await run(cwd, new DeepSeekFsParity(cwd));
  } finally {
...
test("write creates an unseen missing file and returns DeepSeek Harness envelope", async () =>
  fixture(async (cwd, fs) => {
    ...
    assert.equal(
      formatDeepSeekWriteOutput(out.path, out.operation),
      `<path>${join(cwd, "new.txt")}</path>\n<type>file</type>\n<content>\nCreated file\n</content>`,
    );
  }));

test("write refuses blind overwrite until read", async () =>
  fixture(async (cwd, fs) => {
...
test("write detects stale version after read", async () =>
  fixture(async (cwd, fs) => {
...
test("edit requires read and exact unique match by default", async () =>
  fixture(async (cwd, fs) => {
    const path = join(cwd, "a.txt");
    await writeFile(path, "x\nx\n");
    await rejectsMessage(
      fs.edit("a.txt", "x", "y"),
      `edit requires reading "${path}" first — read the file, then retry`,
    );
    await fs.read("a.txt");
    await rejectsMessage(
      fs.edit("a.txt", "x", "y"),
      `old_string matched 2 times in "${path}"; provide a more specific old_string or set replace_all to true`,
    );
    const out = await fs.edit("a.txt", "x", "y", true);
    assert.equal(await readFile(path, "utf8"), "y\ny\n");
    assert.equal(
      formatDeepSeekEditOutput(out.path, true),
      `The file ${path} has been updated. All occurrences were successfully replaced.`,
    );
  }));

test("edit detects stale version before literal matching", async () =>
  fixture(async (cwd, fs) => {
    const path = join(cwd, "a.txt");
    ...
    await rejectsMessage(
      fs.edit("a.txt", "before", "after"),
      `cannot edit "${path}": file changed since it was read — re-read the file, then retry`,
    );
  }));

test("edit preserves CRLF while matching normalized LF strings", async () =>
  fixture(async (cwd, fs) => {
    const path = join(cwd, "a.txt");
...
test("read line counting matches DeepSeek Harness trailing-newline behavior", async () =>
  fixture(async (cwd, fs) => {
    const path = join(cwd, "a.txt");
    ...
    assert.equal(
      formatDeepSeekReadOutput(out),
      `<path>${path}</path>\n<type>file</type>\n<content>\n1: a\n2: b\n\n(End of file - total 2 lines)\n</content>`,
    );
  }));

test("successful mutation updates observation so another edit can follow without reread", async () =>
  fixture(async (cwd, fs) => {
...
test("read absent records absence; write can then create", async () =>
  fixture(async (cwd, fs) => {
...
test(
  "write preserves POSIX file mode on replacement",
...
test(
  "symlink alias keeps identity and edits target without replacing symlink",
  { skip: process.platform === "win32" },
  async () =>
    fixture(async (cwd, fs) => {
...
test("parallel writes from the same observation use the same CAS intent", async () =>
  fixture(async (cwd, fs) => {
    ...
    assert.ok(rejected);
    assert.match(
      String(rejected.reason?.message),
      /file changed since it was read — re-read the file, then retry$/,
    );
  }));

test("parallel creates are no-clobber: one creates and the other is not-observed", async () =>
  fixture(async (cwd, fs) => {
    ...
    assert.equal(
      ...
      `cannot overwrite existing "${path}" without reading it first — read the file, then retry`,
    );
  }));

test("parallel edits from the same observation do not chain through updated observation", async () =>
  fixture(async (cwd, fs) => {
...
test("str_replace_editor view observation is shared with generic edit", async () =>
  fixture(async (cwd, fs) => {
...
test("generic read observation is shared with str_replace_editor mutation", async () =>
  fixture(async (cwd, fs) => {
...
test("str_replace_editor edit-intent errors are raw Harness errors", async () =>
  fixture(async (cwd, fs) => {
    const path = join(cwd, "unread.txt");
    await writeFile(path, "alpha\n");
    await rejectsMessage(
      fs.editorReplace(path, "alpha", "A"),
      `edit requires reading "${path}" first`,
    );
  }));

test("str_replace_editor duplicate error reports every occurrence line", async () =>
  fixture(async (cwd, fs) => {
...
test("str_replace_editor create observes the created version for a following generic edit", async () =>
  fixture(async (cwd, fs) => {
