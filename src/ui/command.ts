import type { EditModesSettings, ModeResolution, SessionToolMode, SessionToolSurface } from "../config/types.ts";
...
export async function openSettingsDialog(input: {
  ...
  return input.ctx.ui.custom(
    (tui: any, theme: any, _kb: any, done: (result: DialogResult) => void) => new SettingsDialog(
      { sessionMode: input.sessionMode, sessionSurface: input.sessionSurface, settings: input.settings },
      input.modelLabel,
