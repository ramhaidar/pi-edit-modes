...
function truncate(text: string): string {
  return text.length <= MAX_OUTPUT_CHARS
    ? text
    : text.slice(0, MAX_OUTPUT_CHARS) + TRUNCATED_MESSAGE;
}
...
function renderResult(result: any, options: any, theme: any, context?: any) {
  ...
  const resultDetails = result.details as
    { target?: string; action?: CompactFileAction; diff?: string } | undefined;
  ...
  if (!isError && resultDetails && Object.hasOwn(resultDetails, "diff")) {
    ...
    if (state?.callComponent) {
      if (resultDetails.target && resultDetails.action)
        updateDeepSeekHeader(
          state.callComponent,
          theme,
          resultDetails.action,
          resultDetails.target,
        );
