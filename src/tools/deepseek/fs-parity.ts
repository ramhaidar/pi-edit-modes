...
function isAbort(error: unknown): boolean {
  return error instanceof Error && (error.name === "AbortError" || ("code" in error && (error as NodeJS.ErrnoException).code === "ABORT_ERR"));
}
...
async function resolveTarget(cwd: string, path: string): Promise<DeepSeekTarget> {
  if (path.trim().length === 0) throw new DeepSeekFsError("file_path must be a non-empty string", "FS_NOT_FOUND");
  ...
  try {
    ...
    if (error instanceof Error && "code" in error && (error as NodeJS.ErrnoException).code === "ENOTDIR") {
      throw new DeepSeekFsError(`cannot resolve "${displayPath}": a parent path segment is not a directory`, "FS_NOT_FOUND", { cause: error });
    }
    ...
    try {
      ...
      if (process.platform === "win32") {
        ...
        if (!parentInfo.isDirectory()) {
          throw new DeepSeekFsError(`cannot resolve "${displayPath}": a parent path segment is not a directory`, "FS_NOT_FOUND");
        }
      }
...
function decodeUtf8(buffer: Uint8Array, verb: "read" | "edit", displayPath: string): string {
  try {
    ...
    throw new DeepSeekFsError(`cannot ${verb} "${displayPath}": invalid UTF-8 text`, "FS_NOT_TEXT", { cause: error });
  }
}
...
export function restoreDeepSeekLineEndings(content: string, lineEndings: "LF" | "CRLF"): string {
  return lineEndings === "LF" ? content : normalizeDeepSeekLineEndings(content).split("\n").join("\r\n");
}
...
export function applyDeepSeekLiteralEdit(
  ...
  if (oldNorm.length === 0) throw new DeepSeekFsError("old_string must be a non-empty string", "FS_EDIT_NOT_FOUND");
  const newNorm = normalizeDeepSeekLineEndings(newString);
  const replacements = countOccurrences(content, oldNorm);
  if (replacements === 0) throw new DeepSeekFsError(`old_string was not found in "${displayPath}"`, "FS_EDIT_NOT_FOUND");
...
function remediate(error: unknown): unknown {
  ...
  if (error.code === "FS_STALE_VERSION") {
    return new DeepSeekFsError(`${error.message} — re-read the file, then retry`, error.code, { cause: error });
  }
  if (error.code === "FS_NOT_OBSERVED") {
    return new DeepSeekFsError(`${error.message} — read the file, then retry`, error.code, { cause: error });
  }
  return error;
}
...
async function atomicWrite(
  ...
  const stagingDir = resolve(directory, `.${basename(targetKey)}.${process.pid}.${randomUUID()}.tmpdir`);
  ...
  try {
    ...
    if (process.platform === "win32" && mode !== undefined) await copyFileDaclWin32(targetKey, tempPath);
    ...
    if (createIfAbsent) {
      try {
        ...
        try { collision = await lstat(targetKey); } catch (metadataError) {
          if (!isMissing(metadataError)) throw metadataError;
        }
        if (collision) {
          if (!collision.isFile()) throw new DeepSeekFsError(`cannot write "${displayPath}": not a regular file`, "FS_NOT_REGULAR_FILE", { cause: error });
          throw new DeepSeekFsError(`cannot overwrite existing "${displayPath}" without reading it first`, "FS_NOT_OBSERVED", { cause: error });
        }
...
function buildReadWindow(content: string, displayPath: string, offset: number, limit: number): Omit<DeepSeekReadResult, "path" | "offset"> {
  ...
  if (offset > lines.length && !(lines.length === 0 && offset === 1)) {
    throw new DeepSeekFsError(`offset ${offset} is out of range for "${displayPath}" (${lines.length} lines)`, "FS_NOT_FOUND");
  }
  ...
  for (let i = offset - 1; i < lines.length && selected.length < limit; i += 1) {
    const raw = lines[i] ?? "";
    const text = raw.length > DEEPSEEK_READ_MAX_LINE_LENGTH
...
export class DeepSeekFsParity {
  ...
  async read(filePath: string, offset = 1, limit = DEEPSEEK_READ_LIMIT, signal?: AbortSignal): Promise<DeepSeekReadResult> {
    if (filePath.trim().length === 0) throw new Error("file_path must be a non-empty string");
    if (!Number.isFinite(offset) || !Number.isInteger(offset) || offset < 1) throw new Error("offset must be a positive integer");
    if (!Number.isFinite(limit) || !Number.isInteger(limit) || limit < 1) throw new Error("limit must be a positive integer");
    if (limit > DEEPSEEK_READ_LIMIT) throw new Error(`limit must be less than or equal to ${DEEPSEEK_READ_LIMIT}`);
    ...
    if (info.type !== "file") throw new DeepSeekFsError(`cannot read "${target.displayPath}": not a regular file`, "FS_NOT_REGULAR_FILE");
    ...
    const window = buildReadWindow(content, target.displayPath, offset, limit);
    this.observations.set(target.targetKey, { kind: "present", version: info.version });
    return { path: target.displayPath, offset, ...window };
  }

  async write(filePath: string, content: string, signal?: AbortSignal): Promise<{ path: string; operation: "create" | "update"; before: string | null; after: string }> {
    if (filePath.trim().length === 0) throw new Error("file_path must be a non-empty string");
    const target = await this.target(filePath);
    ...
    const intent = observed?.kind === "present"
    ...
    return this.withLock(target.targetKey, async () => {
      try {
        ...
        if (existing && existing.type !== "file") throw new DeepSeekFsError(`cannot write "${target.displayPath}": not a regular file`, "FS_NOT_REGULAR_FILE");
        if (intent.kind === "replaceIfVersion") {
          if (!existing) throw new DeepSeekFsError(`cannot write "${target.displayPath}": file no longer exists`, "FS_STALE_VERSION");
          if (existing.version !== intent.version) throw new DeepSeekFsError(`cannot write "${target.displayPath}": file changed since it was read`, "FS_STALE_VERSION");
        } else if (existing) {
          throw new DeepSeekFsError(`cannot overwrite existing "${target.displayPath}" without reading it first`, "FS_NOT_OBSERVED");
        }
        const diffable = Boolean(existing) && Buffer.byteLength(content, "utf8") < DIFF_BASIS_MAX_BYTES;
        const before = diffable ? await readForDiff(target.targetKey, signal) : null;
        await atomicWrite(target.targetKey, target.displayPath, content, existing?.mode, signal, intent.kind === "createIfAbsent");
        const afterInfo = await probe(target.targetKey);
        if (afterInfo) this.observations.set(target.targetKey, { kind: "present", version: afterInfo.version });
        return { path: target.displayPath, operation: existing ? "update" : "create", before, after: normalizeDeepSeekLineEndings(content) };
      } catch (error) {
  ...
  async edit(
    ...
    const observed = this.observations.get(target.targetKey);
    if (!observed) throw remediate(new DeepSeekFsError(`edit requires reading "${target.displayPath}" first`, "FS_NOT_OBSERVED"));
    if (observed.kind === "absent") throw new DeepSeekFsError(`cannot edit "${target.displayPath}": not found`, "FS_NOT_FOUND");
    const expectedVersion = observed.version;

    return this.withLock(target.targetKey, async () => {
      try {
        const existing = await probe(target.targetKey);
        if (!existing) throw new DeepSeekFsError(`cannot edit "${target.displayPath}": file changed since it was read`, "FS_STALE_VERSION");
        if (existing.type !== "file") throw new DeepSeekFsError(`cannot edit "${target.displayPath}": not a regular file`, "FS_NOT_REGULAR_FILE");
        if (existing.version !== expectedVersion) throw new DeepSeekFsError(`cannot edit "${target.displayPath}": file changed since it was read`, "FS_STALE_VERSION");
        throwIfAborted(signal, "edit");
        const bytes = await readRaw(target.targetKey, signal);
        throwIfAborted(signal, "edit");
        if (bytes.includes(0)) throw new DeepSeekFsError(`cannot edit "${target.displayPath}": binary file`, "FS_NOT_TEXT");
        ...
        const edited = applyDeepSeekLiteralEdit(before, oldString, newString, replaceAll, target.displayPath);
        const storage = restoreDeepSeekLineEndings(edited.content, lineEndings);
        await atomicWrite(target.targetKey, target.displayPath, storage, existing.mode, signal, false);
        const afterInfo = await probe(target.targetKey);
        if (afterInfo) this.observations.set(target.targetKey, { kind: "present", version: afterInfo.version });
  ...
  async editorView(path: string, signal?: AbortSignal): Promise<{ target: DeepSeekTarget; info: DeepSeekPathInfo; content?: string }> {
    const target = await this.target(path);
    throwIfAborted(signal, "read");
    const info = await probe(target.targetKey);
    if (!info) {
      this.observations.set(target.targetKey, { kind: "absent" });
      throw new DeepSeekFsError(`The path ${target.displayPath} does not exist. Please provide a valid path.`, "FS_NOT_FOUND");
    }
    if (info.type === "directory") return { target, info };
    if (info.type !== "file") {
      throw new DeepSeekFsError(`cannot view "${target.displayPath}": not a regular file or directory`, "FS_NOT_REGULAR_FILE");
    }
    const bytes = await readRaw(target.targetKey, signal);
    throwIfAborted(signal, "read");
    if (bytes.subarray(0, BINARY_SAMPLE_BYTES).includes(0)) {
  ...
  async editorCreate(path: string, content: string, signal?: AbortSignal): Promise<{ path: string; before: string; after: string }> {
    const target = await this.target(path);
    throwIfAborted(signal, "write");
    ...
    return this.withLock(target.targetKey, async () => {
      ...
      if (existing && existing.type !== "file") {
        throw new DeepSeekFsError(`cannot write "${target.displayPath}": not a regular file`, "FS_NOT_REGULAR_FILE");
      }
      if (intent.kind === "replaceIfVersion") {
        if (!existing) throw new DeepSeekFsError(`cannot write "${target.displayPath}": file no longer exists`, "FS_STALE_VERSION");
        if (existing.version !== intent.version) {
          throw new DeepSeekFsError(`cannot write "${target.displayPath}": file changed since it was read`, "FS_STALE_VERSION");
        }
      } else if (existing) {
        throw new DeepSeekFsError(`cannot overwrite existing "${target.displayPath}" without reading it first`, "FS_NOT_OBSERVED");
      }
  ...
  async editorReplace(
    ...
    const observed = this.observations.get(target.targetKey);
    if (!observed) throw new DeepSeekFsError(`edit requires reading "${target.displayPath}" first`, "FS_NOT_OBSERVED");
    if (observed.kind === "absent") throw new DeepSeekFsError(`cannot edit "${target.displayPath}": not found`, "FS_NOT_FOUND");
    const expectedVersion = observed.version;
    if (oldString === undefined) throw new Error("Parameter `old_str` is required for command: str_replace");
    if (oldString.length === 0) throw new Error("Parameter `old_str` is empty for command: str_replace");
    ...
    if (info.type === "directory") {
      throw new DeepSeekFsError(`The path ${target.displayPath} is a directory and only the \`view\` command can be used on directories`, "FS_NOT_REGULAR_FILE");
    }
    if (info.type !== "file") throw new DeepSeekFsError(`cannot edit "${target.displayPath}": not a regular file`, "FS_NOT_REGULAR_FILE");
    const bytes = await readRaw(target.targetKey, signal);
    if (bytes.subarray(0, BINARY_SAMPLE_BYTES).includes(0)) {
      throw new DeepSeekFsError(`cannot read "${target.displayPath}": binary file`, "FS_NOT_TEXT");
    }
    ...
    return this.withLock(target.targetKey, async () => {
      ...
      if (!current) throw new DeepSeekFsError(`cannot write "${target.displayPath}": file no longer exists`, "FS_STALE_VERSION");
      if (current.type !== "file") throw new DeepSeekFsError(`cannot write "${target.displayPath}": not a regular file`, "FS_NOT_REGULAR_FILE");
  ...
  async editorInsert(
    ...
    if (info.type !== "file") throw new DeepSeekFsError(`cannot insert into "${target.displayPath}": not a regular file`, "FS_NOT_REGULAR_FILE");
    const bytes = await readRaw(target.targetKey, signal);
    if (bytes.subarray(0, BINARY_SAMPLE_BYTES).includes(0)) {
      throw new DeepSeekFsError(`cannot read "${target.displayPath}": binary file`, "FS_NOT_TEXT");
    }
    ...
    const after = [...lines.slice(0, insertLine), ...newString.split("\n"), ...lines.slice(insertLine)].join("\n");
  ...
  async observeAbsolutePath(path: string, signal?: AbortSignal): Promise<{ target: DeepSeekTarget; info?: DeepSeekPathInfo }> {
    const target = await this.target(path);
    throwIfAborted(signal, "read");
    const info = await probe(target.targetKey);
    this.observations.set(target.targetKey, info ? { kind: "present", version: info.version } : { kind: "absent" });
    return { target, info };
  }
